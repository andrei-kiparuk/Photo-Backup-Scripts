...
source_dir = "/Volumes/T9-1/iCloud"
dest_base_dir = "/Volumes/T9-1/toimport/sorted"
...
def get_folder_date(filepath):
    # Extract the folder date from the path (e.g., /Volumes/T7/donotimport/UnsortedPhotos/2015/06/02)
...
def get_content_created_date(filepath, is_video=False):
    # Get Content Created date using exiftool
    with exiftool.ExifToolHelper() as et:
        try:
            metadata = et.get_metadata(filepath)[0]
            # Try content-specific tags, prioritizing video-specific tags if applicable
            ...
            # Prioritize video-specific tags if the file is a video
            if is_video:
                content_tags = [
...
def get_unique_filename(destination_folder, filename):
    # If the file already exists in the destination, append _NUMBER to the filename
    ...
    while os.path.exists(new_path):
        ...
        counter += 1
    return new_path, new_filename if counter > 1 else filename

def get_correct_folder(earliest_date):
    # Construct the correct folder path based on the earliest date
    year = earliest_date.strftime("%Y")
    month = earliest_date.strftime("%m")
    day = earliest_date.strftime("%d")
...
def organize_files():
    ...
    for root, _, files in os.walk(source_dir):
        for filename in files:
            ...
            content_created_date = get_content_created_date(filepath, is_video)
            ...
            # Handle conflicts by renaming the file if necessary
            new_path, new_filename = get_unique_filename(correct_folder, filename)

            # Move the file
            try:
                shutil.move(filepath, new_path)
                print(f"Moved {filename} from {current_folder} to {correct_folder} as {new_filename}")
            except Exception as e:
